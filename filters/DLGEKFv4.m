...
% P(10:12);     % velocity in navigation frame
        ...
        %%% Equation test
        if n==Nt-100
            omb=x_maj(1:3);           % omega in body coordinates
            sb=resProp.s;             % specific force at origin of body frame in body coordinates
            omdotb=resProp.omega_dot; % omega dot in body coordinates
            for i=1:(length(u_n)/3)                
                rb = settings.r(:,i);      % first sensor coordinates in body frame
                fb=u_n((i-1)*3+(1:3));              % first sensor measured accelerations in body coordinates
                test=sb+cross(omb,cross(omb,rb))+cross(omdotb,rb)-fb;
                disp (num2str([fb sb omb omdotb rb test]))
                % disp (sprintf('%10.6f %10.6f %10.6f %10.6f %10.6f \n', fb, sb, omb, omdotb, rb, test))
            end
        end
        % Mean propagation
        % n|n-1
